...
const MissionVisionValues = () => {
    ...
    const heroSubtitle = settings?.siteIdentity?.siteName
    ...
    return (
        <div className="page-wrapper">
            <PageHero
                title={content.heroTitle || defaultContent.heroTitle}
            ...
            <div className="container" style={{ padding: '80px 20px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', marginBottom: '80px' }}>
                    <div>
                        <h2 style={{ fontSize: '2.5rem', marginBottom: '1.5rem', color: 'var(--accent-secondary)' }}>{content.missionTitle || defaultContent.missionTitle}</h2>
                        <p style={{ fontSize: '1.1rem', lineHeight: '1.8' }}>{content.missionText || defaultContent.missionText}</p>
                    </div>
                    <div>
                        <h2 style={{ fontSize: '2.5rem', marginBottom: '1.5rem', color: 'var(--accent-secondary)' }}>{content.visionTitle || defaultContent.visionTitle}</h2>
                        <p style={{ fontSize: '1.1rem', lineHeight: '1.8' }}>{content.visionText || defaultContent.visionText}</p>
                    </div>
                </div>

                <div className="grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2rem' }}>
                    {(content.values || defaultContent.values).map((v, i) => {
