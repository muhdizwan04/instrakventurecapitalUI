...
const AboutUs = () => {
    // --- Data Fetching (Preserving Admin Logic) ---
    
    // 1. Mission Content
    const defaultMission = {
        heroTitle: 'About Us',
        heroSubtitle: 'The foundational pillars, leadership, and partners of Instrak Venture Capital Berhad.',
    ...
    const { content: missionContent } = usePageContent('about', defaultMission);
    ...
    const defaultPartners = {
        pageHeader: { title: 'Strategic Partners', subtitle: '' },
        partners: [{
    ...
    const customSections = partnersContent.customSections || [];

    // Static highlights for partners section styling
    const highlights = [
    ...
    return (
        <div className="page-wrapper bg-gray-50">
            <PageHero
                title={missionContent.heroTitle || defaultMission.heroTitle}
                subtitle={missionContent.heroSubtitle || defaultMission.heroSubtitle}
            />

            {/* --- SECTION 1: MISSION & VISION (Premium Redesign) --- */}
            <section id="mission" style={{ padding: '100px 0', scrollMarginTop: '60px' }}>
                <div className="container">
                    ...
                    <div style={{ 
                        ...
                        <div style={{ position: 'relative', zIndex: 10, maxWidth: '800px' }}>
                            ...
                            <h2 style={{ fontSize: '2.5rem', marginBottom: '1.5rem', lineHeight: '1.2', color: '#FFFFFF' }}>{missionContent.missionTitle || defaultMission.missionTitle}</h2>
                            <p style={{ fontSize: '1.25rem', lineHeight: '1.8', opacity: 0.9, marginBottom: '2rem' }}>
                                {missionContent.missionText || defaultMission.missionText}
                            </p>
                            
                            <div style={{ height: '1px', background: 'rgba(255,255,255,0.2)', margin: '2rem 0' }}></div>

                            <h3 style={{ fontSize: '1.5rem', color: '#D4AF37', marginBottom: '1rem' }}>{missionContent.visionTitle || defaultMission.visionTitle}</h3>
                             <p style={{ fontSize: '1.1rem', lineHeight: '1.8', opacity: 0.8 }}>
                                {missionContent.visionText || defaultMission.visionText}
                            </p>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
                        {(missionContent.values || defaultMission.values).map((v, i) => {
            ...
            {/* --- SECTION 2: BOARD OF DIRECTORS (Executive Redesign) --- */}
            <section id="board" style={{ padding: '100px 0', background: '#F8FAFC', scrollMarginTop: '60px' }}>
                <div className="container">
                    <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
                        <h6 style={{ color: '#D4AF37', textTransform: 'uppercase', letterSpacing: '2px', fontWeight: 'bold' }}>Leadership</h6>
                        <h2 className="section-title" style={{ marginTop: '0.5rem', color: '#1A365D' }}>Board of Directors</h2>
                        <p style={{ maxWidth: '600px', margin: '1rem auto 0', color: '#64748B' }}>
                            Guided by seasoned leaders with a commitment to integrity, compliance, and industrial excellence.
                        </p>
                    </div>
            ...
            {/* --- SECTION 3: STRATEGIC PARTNERS (Trust & Tombstone) --- */}
            <div id="partners" style={{ scrollMarginTop: '80px' }}>
                ...
                {/* 3c. Banking & Insurance Partners */}
                <section style={{ padding: '100px 0', background: '#FFFFFF' }}>
                    <div className="container">
                        <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
                            <h2 className="section-title">Strategic Banking & Insurance</h2>
                        </div>

                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '2rem' }}>
                            {/* Chubb */}
                            {partners.map((partner, i) => (
                                <div key={i} style={{ border: '1px solid #E2E8F0', borderRadius: '16px', padding: '3rem', position: 'relative' }}>
                ...
                {/* 3d. Compliance Footer */}
                <section style={{ background: '#F1F5F9', padding: '60px 0' }}>
                    <div className="container">
                         <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
                            <h3 style={{ fontSize: '1.25rem', color: '#1A365D', marginBottom: '1rem' }}>{trustContent.title}</h3>
                            <p style={{ color: '#64748B', marginBottom: '2rem', lineHeight: '1.6' }}>{trustContent.description}</p>
